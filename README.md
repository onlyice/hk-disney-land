...
- **最佳到访日**：判断「哪天/哪月去人少」。展示按星期、月份、年份的整体拥挤指数，
  最近每日拥挤指数，以及平均最繁忙的项目排行；并高亮一周/一年中最空与最挤的时段。

## 关于历史数据的重要说明

queue-times 的**免费 API 只提供实时数据，没有历史数据接口**。它虽然保存了多年历史，
但对外只在网站统计页公开到**「日 / 星期 / 月」级别的拥挤指数**，**不提供按时段（小时/分钟）
的日内历史**，单设施的日内历史页面还有反爬保护。

因此本项目分两条路拿数据：

1. **日内时段数据（实时面板、历史分析、路线规划所需）**——官方拿不到历史，只能**靠本项目
   从现在起每 5 分钟自行采集并积累**。所以请**尽早开始采集**。
2. **「最佳到访日」用的拥挤日历**——来自 queue-times 统计页内嵌的聚合数据（按星期/月份/日期
   的拥挤指数），可一次性获取并每天刷新。⚠️ 这依赖其页面结构，若官方改版解析可能失效；
   且仅为天级粒度，不能替代上面的日内时段数据。
...
| `GET /api/crowd` | 最佳到访日 / 拥挤日历数据 |
| `POST /api/crowd/refresh` | 立即采集/刷新拥挤日历 |
